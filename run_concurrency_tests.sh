...
# 设置环境变量
VENV_NAME="aider_env"
TEST_FILE="tests/browser/test_fastapi_concurrency.py"
TEST_METHOD=""
LOG_FILE="concurrency_test.log"
...
# 添加初步日志输出
echo "========== 开始执行测试 $(date) ==========" >> "$LOG_FILE"

# 设置测试命令
TEST_CMD="pytest -xvs"
if [ ! -z "$TEST_METHOD" ]; then
    TEST_CMD="$TEST_CMD $TEST_FILE::TestFastAPIConcurrency::$TEST_METHOD"
    print_info "将只运行测试方法: $TEST_METHOD"
    echo "运行测试方法: $TEST_METHOD" >> "$LOG_FILE"
else
    TEST_CMD="$TEST_CMD $TEST_FILE"
    print_info "将运行所有测试方法"
    echo "运行所有测试方法" >> "$LOG_FILE"
fi
...
# 提取错误信息到单独的错误日志
grep -E "^(E|ERROR|FAILED|Exception|Error|Traceback)" "$LOG_FILE" > "error_summary.log"
if [ -s "error_summary.log" ]; then
    print_info "错误摘要已保存到 error_summary.log"
    echo -e "\n关键错误信息:"
    cat "error_summary.log" | head -n 20
    if [ $(wc -l < "error_summary.log") -gt 20 ]; then
        echo "... 更多错误信息请查看 error_summary.log ..."
    fi
fi

# 输出测试结果
if [ $TEST_RESULT -eq 0 ]; then
    print_success "所有测试通过! FastAPI并发工作目录隔离测试成功"
else
